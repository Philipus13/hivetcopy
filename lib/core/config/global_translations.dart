...
const List<String> _kSupportedLanguages = ["en", "id"];
...
class GlobalTranslations {
  ...
  Future<Null> setNewLanguage([String? newLanguage]) async {
    ...
    String language = newLanguage != null ? newLanguage : 'id';
    language = (await SharedPrefUtil.getLanguage(prefs))!;
  ...
  Future<Null> init() async {
    prefs = await SharedPreferences.getInstance();
    String? language = await SharedPrefUtil.getLanguage(prefs);
    if (language == null) {
      await setNewLanguage('id');
    } else {
